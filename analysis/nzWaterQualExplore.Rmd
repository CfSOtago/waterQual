 ...
 * `r uniqueN(wqdt$sID)` sites

This is hardly trend or raw data - it is a single record per site with indicator of change via a trend slope coefficient or a single % change value. There ae also data quality & missing data issues (see Statistical Annex). It's noticeable for example that, irrespective of the indicatr, over half of stations were thought to have insufficient data to determine trend direction. This was true of 70% of stations for [ECOLI](https://www.stats.govt.nz/indicators/river-water-quality-escherichia-coli) and 78% for the Macroinvertebrate Community Index ([MCI](https://www.stats.govt.nz/indicators/river-water-quality-macroinvertebrate-community-index)).
...
```{r plotSites, fig.height = 10, fig.cap="Site level trend plots"}
...
tn <- wqdt[!is.na(AnalysisNote) & AnalysisNote != "ok", .(sID, npID, AnalysisNote)]
